...
func TestSelectOutputMode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := format.SelectOutputMode(tt.jsonFlag, tt.tsvFlag)
			if tt.wantErr {
				if err == nil {
					t.Fatalf("SelectOutputMode(%v, %v) returned nil error", tt.jsonFlag, tt.tsvFlag)
				}
				return
			}
			if err != nil {
				t.Fatalf("SelectOutputMode(%v, %v) returned unexpected error: %v", tt.jsonFlag, tt.tsvFlag, err)
			}
			if got != tt.want {
				t.Fatalf("SelectOutputMode(%v, %v) = %q, want %q", tt.jsonFlag, tt.tsvFlag, got, tt.want)
			}
		})
	}
}
