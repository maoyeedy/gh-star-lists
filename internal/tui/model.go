...
func (m model) handleEnter() (tea.Model, tea.Cmd) {
	if m.active == paneList {
		...
		m.repoCursor = 0
		m.loading = true
		return m, loadReposCmd(m.ctx, m.svc, focused.ID)
	}
...
func (m model) handleRefresh() (tea.Model, tea.Cmd) {
	...
	if m.active == paneRepo && m.focusedList != nil {
		return m, loadReposCmd(m.ctx, m.svc, m.focusedList.ID)
	}
	m.lists = nil
	m.repos = nil
	m.focusedList = nil
...
func (m model) renderLayout() string {
	...
	contentH := m.height - 2 // reserve 1 line header + 1 line footer
...
func (m model) renderFooter() string {
	hints := "enter:select  o:open  s:sort  ctrl+r:refresh  ?:help  q:quit"
	if m.active == paneRepo {
		hints = "enter/o:open  esc:back  s:sort  ctrl+r:refresh  ?:help  q:quit"
	}
