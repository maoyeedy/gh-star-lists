...
func TestMouseClickFocusesPane(t *testing.T) {
	...
	// leftW at width 120: fixed 24. Repo pane starts after x=24.
...
func TestHoverWheelScrollsListPane(t *testing.T) {
	...
	// g.sep1Col at width=120: fixed leftW = 24.
...
func TestHoverWheelScrollsRepoPane(t *testing.T) {
	...
	// g.sep1Col at width=120: leftW = 24, sep1Col = 24.
	// X=50 is in the repo pane (> sep1Col=24).
	before := m.repoCursor
...
func TestSinglePaneMouseUsesActivePane(t *testing.T) {
	t.Parallel()
	svc := threeListsSvc()
	m := newTestModel(svc)
	m = update(m, listsLoadedMsg{lists: svc.lists})
	...
	m2 := update(m, click)
	if m2.active != paneRepo {
		t.Errorf("active = %v after single-pane repo click, want paneRepo", m2.active)
	}
	if m2.repoCursor != 1 {
		t.Errorf("repoCursor = %d after single-pane repo click, want 1", m2.repoCursor)
	}

	m2.active = paneList
	m2.listCursor = 1
	wheel := tea.MouseWheelMsg{X: 70, Y: 5, Button: tea.MouseWheelUp}
	m3 := update(m2, wheel)
	if m3.listCursor != 0 {
		t.Errorf("listCursor = %d after single-pane list wheel, want 0", m3.listCursor)
	}
}
